...
{	
	...
	override function create()
	{
		...
		var bg:FlxSprite = new FlxSprite(-80).loadGraphic(Paths.image('splashes/menuBG'));
		...
		flip = new FlxSprite().loadGraphic(Paths.image('splashes/menuFlip'));
		flip.antialiasing = ClientPrefs.globalAntialiasing;
		add(flip);
		flip.screenCenter();

		barrier = new FlxSprite().loadGraphic(Paths.image('barriers/menuBottom'));
		...
		//for (i in 0...optionShit.length)
			...
			var menuItem:FlxSprite = new FlxSprite(105, 600);
			...
			// FreePlay Mode
			var offset:Float = 100 - (Math.max(optionShit.length, 4) - 4) * 50;
			var menuItem:FlxSprite = new FlxSprite(455, 600);
			menuItem.scale.x = scale;
			menuItem.scale.y = scale;
			...
			// Options
			var offset:Float = 100 - (Math.max(optionShit.length, 4) - 4) * 50;
			var menuItem:FlxSprite = new FlxSprite(1020, 600);
			menuItem.scale.x = scale;
			menuItem.scale.y = scale;
		...
		var versionShit:FlxText = new FlxText(12, FlxG.height - 64, 0, "" + flippyVersion, 12);
